...
<div class="mws-panel grid_8">
	<div class="mws-panel-header" style="height: 46px;">
		<span> {{ trans("messages.management.edit_faq_manager") }} </span>
		<a href="{{URL::to('admin/faqs-manager')}}" class="btn btn-success btn-small align">{{ trans("messages.management.back_to_listing") }} </a>
	</div>
	...
	<div class="mws-panel-body no-padding tab-content">
		...
		<div id="{{ $i }}div" class="tab-pane {{ ($i ==  $language_code )?'active':'' }} ">
			<div class="mws-form-inline">
				<div class="mws-form-row ">
					{{ HTML::decode( Form::label($i.'.question', 'Question<span class="requireRed"> * </span>', ['class' => 'mws-form-label'])) }}
					<div class="mws-form-item">
						...
						<div class="error-message help-inline">
							{{ ($i ==  $language_code ) ? $errors->first('question') : '' }}
						</div>
					</div>
				</div>
				<div class="mws-form-row ">
					{{ HTML::decode( Form::label($i.'_answer', 'Answer<span class="requireRed"> * </span>', ['class' => 'mws-form-label'])) }}
					<div class="mws-form-item">
						{{ Form::textarea("data[$i][".'answer'."]",isset($multiLanguage[$i]['answer'])?$multiLanguage[$i]['answer']:'', ['class' => 'small','id' => 'answer_'.$i]) }}
						<span class="error-message help-inline">
						{{ ($i ==  $language_code ) ? $errors->first('answer') : '' }}
						</span>
					</div>
		...
		<div class="mws-button-row">
			<div class="input" >
				...
				<a href="{{URL::to('admin/faqs-manager/edit-faqs/'.$AdminFaq->id)}}" class="btn primary"><i class=\"icon-refresh\"></i> {{ trans('messages.management.reset') }}</a>
			</div>
		</div>
	</div>
	{{ Form::close() }} 
